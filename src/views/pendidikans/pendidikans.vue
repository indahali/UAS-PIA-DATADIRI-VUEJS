<template>
  <div class="pendidikans">
    <!--<img alt="Vue logo" src="../assets/logo.png"> -->
    <Slider />
    <hr class="my-3">
      <router-link class="btn btn-primary" to="/creatependidikans">Add pendidikan</router-link>

      <Cardpendidikans :pendidikans="pendidikans" />

    <table class="table">
  <thead>
    <tr>
      <th scope="col">Sekolah</th>
      <th scope="col">Nama Sekolah</th>
      <th scope="col">Mulai Tahun</th>
      <th scope="col">Berakhir Tahun</th>
      <th scope="col">Aksi</th>
    </tr>
  </thead>
  <tbody>
    <tr v-for="(pendidikan, index) in pendidikans" :key="index">
      <td>{{ pendidikan.sekolah }}</td>
      <td>{{ pendidikan.ns }}</td>
      <td>{{ pendidikan.mulai_tahun }}</td>
      <td>{{ pendidikan.berakhir_tahun }}</td>
      <td>
        <router-link class="btn btn-success" :to="{name:'Editpendidikans', params:{id:pendidikan.id}}">Edit</router-link>
        <button @click.prevent="pendidikanDelete(pendidikan.id)" class="btn btn-danger">Delete</button>
      </td>
    </tr>
  </tbody>
</table>
  </div>
</template>
<script>
import axios from 'axios'
// @ is an alias to /src
import Slider from "@/components/Slider.vue";
import Cardpendidikans from "@/components/Cardpendidikans.vue";
import { ref, onMounted } from 'vue';
export default {
  name: "pendidikans",
  components: {
    Slider,
    Cardpendidikans,
  },
  setup(){
    let pendidikans = ref([])
    onMounted(() => {
      axios.get('http://127.0.0.1:8000/api/pendidikans')
      .then(response => {
        pendidikans.value = response.data.data
      })
      .catch(error => {
        console.log(error)
      })
    })
    function pendidikanDelete(id){
      axios.delete(`http://127.0.0.1:8000/api/pendidikans/${id}`)
      .then(()=>{
        let z = this.pendidikans.map(pendidikans => pendidikans.id).indexOf(id);
        this.pendidikans.splice(z, 1)
      }).catch(error => {
        console.log(error)
      })
    }
      return {
      pendidikans,
      pendidikanDelete
    }
  }
};
</script>